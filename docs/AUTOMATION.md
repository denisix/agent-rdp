...
| Command | Purpose |
|---------|---------|
| `run` / `run_poll` | Launch a process; wait, or stream its output incrementally. The child script is assembled by `New-ChildScript`: a prelude (UTF-8 console output without BOM, guarded so a console-less child cannot die on it; `$ProgressPreference='SilentlyContinue'`; `$ErrorActionPreference='Stop'`; `$env:AGENT_RDP_AGENT_PID`), then the user script inside `try { … } catch { … }` whose catch prints the exception chain as plain text to stderr and exits 1, with `$LASTEXITCODE` preserved for native commands. Scripts with a `param` block or `using` statements (detected with the PowerShell parser, `Get-ChildScriptShape`) get the prelude only; a script with parse errors is refused before launch with `parse_error` (line/column per error) when the child shell is Windows PowerShell — the agent's own parser — and launched unwrapped for any other `shell`. `Get-ShellKind` decides how the text is delivered: `powershell.exe`/`pwsh.exe` get `-NoProfile -EncodedCommand`, `cmd.exe` gets `/d /s /c "chcp 65001>nul & <line>"` with no parser gate and no wrapper (the `chcp` because a `CreateNoWindow` child gets a fresh console at the OEM codepage while the agent decodes UTF-8), and anything else is refused with `shell_unsupported`. Arguments are quoted for whichever interpreter will re-parse them (so
`command_line` echoes the flag-shaped ones bare and the rest as literals),
and a
**flag-shaped** argument (`^--?[A-Za-z_][A-Za-z0-9_-]*$`, plus the bare `--`
and `--%`) is passed through unquoted: a quoted token is never a parameter
name to PowerShell, so wrapping everything turned `-Filter "x"` into two
positional strings and silently dropped the parameter. For a native `.exe`
the two forms are identical, so nothing that worked before changes. `\-value`
escapes a value that genuinely looks like a flag. Typed arguments are still
not expressible — `-1` arrives as the string `-1` — and belong in the command
string. A redirection to a DOS device (`2>nul`) in text bound for PowerShell is refused with `cmd_syntax` — found in the AST (`FileRedirectionAst`) rather than by matching the .NET exception, whose wording is localized. Every reply carries `command_line` (the command plus each argument exactly as the child parses it: flag-shaped arguments bare, the rest as single-quoted literals) and every error thrown by `Invoke-Run` ends with the same text. Waited runs add `finished_unix`; a finished stream poll adds it too. `wait` takes precedence over `stream`. A detached launch reports `early_exit`/`exit_code` if the child is gone ~250ms after start. Streamed output is captured to files under `%TEMP%\agent-rdp-run` and read back by offset (a read failure is reported in-band and retried next poll); finished entries stay pollable for 10 minutes. Remaining CLIXML on stderr (parse errors, unwrapped scripts) is reduced by the daemon to the text of error/warning records, including `<Obj>` records' `ToString`. A `run` carrying `idempotency_key` uses it as the request id (see Indeterminate Results). A waited child is put in a **job object** immediately after `Process.Start`, so `timeout_ms` kills the whole tree (`Stop-RunTree`) rather than the wrapper shell whose grandchildren would otherwise survive; the kill is verified against the job's `ActiveProcesses` and, on a host that refuses the assignment, against handles taken before the kill from a `Win32_Process` walk (`Get-ProcessDescendant`). Survivors are reported as `kill_failed:` and named. `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE` is deliberately not set, so an agent restart never kills a caller's running command. Residual gap: the child exists for a few milliseconds before it is in the job, so something it spawns in that window can escape |
...
| `cmd_syntax` (`command_failed` from the daemon's view) | The `run` text redirects to a DOS device (`2>nul`, `>con`) but is being handed to PowerShell, which would try to open it as a file; or a `cmd.exe` argument contains a double quote. Nothing was launched |
| `shell_unsupported` (`command_failed` from the daemon's view) | `shell` is not `powershell.exe`, `pwsh.exe` or `cmd.exe`. Nothing was launched |
| `kill_failed` (`command_failed` from the daemon's view) | `run --wait` overran `--process-timeout` and the process tree could **not** be verified dead. Names every survivor by pid and image. The plain timeout message (no `kill_failed:`) means the tree is verifiably gone. Both carry the host CPU load at the kill and whether the command had started |
